...
class home extends Component {
  ...
  componentDidMount() {
    ...
    if (id == undefined) {
      Alert.alert('Error', 'Silahkan Login Dulu');
    }
    let linkLocal =
      'http://' + db.state.linkLocal + '/hidroponik/api/Prototype/' + id;
    let link = '';
    fetch(linkLocal)
  ...
  _renderItem({item, index}) {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          item.navigate.push('prototype', {id: item.prototype_id});
        }}>
        <View
          style={{
            flexDirection: 'row',
          ...
          <Icon2
            name="seedling"
            size={25}
            color={item.title == 'Online' ? 'red' : 'green'}
          />
        </View>
        <View
          style={{
